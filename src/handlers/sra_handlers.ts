...
export class SRAHandlers {
  ...
  public static rootAsync(_req: express.Request, res: express.Response): void {
    const message = `This is the root of the Standard Relayer API.`;
    res.status(HttpStatus.OK).send({ message });
  }
  public static feeRecipients(
    req: express.Request,
    ...
    const paginatedFeeRecipients = paginationUtils.paginate(
      feeRecipients,
      page,
      perPage
    );
  ...
  public async getOrderByHashAsync(
    req: express.Request,
    res: express.Response
  ): Promise<void> {
    const orderIfExists = await this._orderBook.getOrderByHashIfExistsAsync(
      req.params.orderHash
    );
  ...
  public async ordersAsync(
    ...
    const query = {
      ...
      makerAssetData: req.query.makerAssetData
        ? parseUtils.parseAssetDatasStringFromQueryParam(
            req.query.makerAssetData as string
          )
        : undefined,
      takerAssetData: req.query.takerAssetData
        ? parseUtils.parseAssetDatasStringFromQueryParam(
            req.query.takerAssetData as string
          )
        : undefined,
    } as any;
  ...
  public async orderbookAsync(
    ...
    const orderbookResponse = await this._orderBook.getOrderBookAsync(
      page,
      perPage,
      baseAssetData,
      quoteAssetData
  ...
  public async postOrderAsync(
    ...
    const pinResult = await this._orderBook.splitOrdersByPinningAsync([
      signedOrder,
    ]);
  ...
  public async postPersistentOrderAsync(
    req: express.Request,
    res: express.Response
  ): Promise<void> {
    ...
    if (
      apiKey === undefined ||
      !isValidUUID(apiKey) ||
      !OrderBookService.isAllowedPersistentOrders()
    ) {
...
function validateAssetDataIsWhitelistedOrThrow(

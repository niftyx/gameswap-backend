 ...
 */
export function alertOnExpiredOrders(
  expired: APIOrder[],
  details?: string
): void {
  const maxExpirationTimeSeconds =
    Date.now() / ONE_SECOND_MS + MAX_ORDER_EXPIRATION_BUFFER_SECONDS;
  let idx = 0;
  if (
    expired.find((order, i) => {
      idx = i;
      return (
        order.order.expirationTimeSeconds.toNumber() > maxExpirationTimeSeconds
      );
    })
  ) {
    const error = new ExpiredOrderError(
      expired[idx].order,
      MAX_ORDER_EXPIRATION_BUFFER_SECONDS,
      details
    );

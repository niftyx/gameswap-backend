...
export class OrderBookService {
  ...
  public static isAllowedPersistentOrders(): boolean {
    return true;
  }
  public async getOrderByHashIfExistsAsync(
    orderHash: string
  ): Promise<APIOrder | undefined> {
    const signedOrderEntityIfExists = await this._connection.manager.findOne(
      SignedOrderEntity,
      orderHash
    );
  ...
  public async getAssetPairsAsync(
    ...
    const signedOrderEntities = (await this._connection.manager.find(
      SignedOrderEntity
    ...
    if (assetDataA === undefined && assetDataB === undefined) {
      ...
      nonPaginatedFilteredAssetPairs = assetPairsItems.filter(
        containsAssetDataAAndAssetDataB
      );
    } else {
      const assetData = assetDataA || assetDataB;
      const containsAssetData = (assetPair: AssetPairsItem) =>
        assetPair.assetDataA.assetData === assetData ||
        assetPair.assetDataB.assetData === assetData;
      nonPaginatedFilteredAssetPairs = assetPairsItems.filter(
        containsAssetData
      );
    }
    const uniqueNonPaginatedFilteredAssetPairs = _.uniqBy(
      ...
      (assetPair) =>
        `${assetPair.assetDataA.assetData}/${assetPair.assetDataB.assetData}`
    );
  ...
  public async getOrderBookAsync(
    ...
    const bidSignedOrderEntities = orderEntities.filter(
      (o) =>
        o.takerAssetData === baseAssetData &&
        o.makerAssetData === quoteAssetData
    );
    const askSignedOrderEntities = orderEntities.filter(
      (o) =>
        o.takerAssetData === quoteAssetData &&
        o.makerAssetData === baseAssetData
    );
  ...
  public async getOrdersAsync(
    ...
    const { fresh, expired } = orderUtils.groupByFreshness(
      apiOrders,
    ...
    if (ordersFilterParams.isUnfillable === true) {
      ...
      [persistentOrdersCount, persistentOrderEntities] = await Promise.all([
        this._connection.manager.count(PersistentSignedOrderEntity, {
          where: filterObject,
        }),
      ...
      persistentOrders = (persistentOrderEntities as Required<PersistentSignedOrderEntity>[])
        ...
        .filter((apiOrder) => {
          return (
            ...
            unfillableStates.includes(apiOrder.metaData.state)
          );
        });
    }

    // Post-filters (query fields that don't exist verbatim in the order)
    const filteredApiOrders = orderUtils.filterOrders(
      fresh.concat(persistentOrders),
      ordersFilterParams
    );
  ...
  public async getBatchOrdersAsync(
    ...
    const apiOrders = signedOrderEntities.map(
      orderUtils.deserializeOrderToAPIOrder
    );
  ...
  public async addPersistentOrdersAsync(
    ...
    await this._connection
      ...
      .save(persistentOrders);
  }
  public async splitOrdersByPinningAsync(
    signedOrders: SignedOrder[]
  ): Promise<PinResult> {
  ...
  private async _addOrdersAsync(
    signedOrders: SignedOrder[],
    pinned: boolean
  ): Promise<AcceptedOrderInfo[]> {
    if (this._meshClient) {
      const { rejected, accepted } = await this._meshClient.addOrdersAsync(
        signedOrders,
        pinned
      );

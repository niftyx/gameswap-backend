...
export class WebsocketService {
  ...
  private readonly _requestIdToSocket: Map<
    string,
    WrappedWebSocket
  ...
  private static _decodedContractAndAssetData(
    assetData: string
  ): { assetProxyId: string; data: string[] } {
    let data: string[] = [assetData];
    const decodedAssetData = assetDataUtils.decodeAssetDataOrThrow(assetData);
    if (orderUtils.isMultiAssetData(decodedAssetData)) {
      for (const nested of (decodedAssetData as any).nestedAssetData) {
        data = [
          ...data,
          ...WebsocketService._decodedContractAndAssetData(nested).data,
        ];
      }
      ...
      data = [
        ...
        ((decodedAssetData as Exclude<
          typeof decodedAssetData,
          MultiAssetDataWithRecursiveDecoding
        >) as any).tokenAddress,
      ];
    }
  ...
  private static _matchesOrdersChannelSubscription(
    ...
    if (
      opts.traderAssetData &&
      makerAssetDataTakerAssetData.includes(opts.traderAssetData)
    ) {
      return true;
    }
    ...
    const makerContractAndAssetData = WebsocketService._decodedContractAndAssetData(
      makerAssetData
    );
    const takerContractAndAssetData = WebsocketService._decodedContractAndAssetData(
      takerAssetData
    );
    if (
  ...
  constructor(
    ...
    this._meshClient
      .subscribeToOrdersAsync((e) =>
        this.orderUpdate(meshUtils.orderInfosToApiOrders(e))
      )
  ...
  public orderUpdate(apiOrders: APIOrder[]): void {
    ...
    for (const order of allowedOrders) {
      ...
      for (const [requestId, subscriptionOpts] of this
        ._requestIdToSubscriptionOpts) {
        if (
  ...
  private _processMessage(ws: WrappedWebSocket, data: WebSocket.Data): void {
    ...
    schemaUtils.validateSchema(
      message,
      schemas.relayerApiOrdersChannelSubscribeSchema
    );

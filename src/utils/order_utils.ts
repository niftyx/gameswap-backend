...
import { MAX_TOKEN_SUPPLY_POSSIBLE, ONE_SECOND_MS } from "../constants";
...
export const orderUtils = {
  isIgnoredOrder: (
    ...
    return (
      ...
      orderUtils.includesTokenAddresses(
  ...
  isFreshOrder: (
    ...
    return (
      apiOrder.order.expirationTimeSeconds.toNumber() >
  ...
  compareOrderByFeeRatio: (
    ...
    return orderA.expirationTimeSeconds.comparedTo(
      orderB.expirationTimeSeconds
    );
  },
  includesTokenAddresses: (
    assetData: string,
    ...
    if (orderUtils.isMultiAssetData(decodedAssetData)) {
      ...
      ] of decodedAssetData.nestedAssetData.entries()) {
        if (
          orderUtils.includesTokenAddresses(
            nestedAssetDataElement,
            tokenAddresses
      ...
      return (
        tokenAddresses.find((a) => a === decodedAssetData.tokenAddress) !==
        undefined
      );
    }
    return false;
  ...
  deserializeOrder: (
    ...
    const signedOrder: SignedOrder = {
      ...
      expirationTimeSeconds: new BigNumber(
        signedOrderEntity.expirationTimeSeconds
      ),
  ...
  deserializeOrderToAPIOrder: (
    signedOrderEntity:
      ...
      | Required<PersistentSignedOrderEntity>
  ): APIOrderWithMetaData => {
    ...
    const createdAt = (signedOrderEntity as PersistentSignedOrderEntity)
      .createdAt;
    const metaData: APIOrderMetaData = {
      orderHash: signedOrderEntity.hash,
      remainingFillableTakerAssetAmount: new BigNumber(
        signedOrderEntity.remainingFillableTakerAssetAmount
      ),
      state,
      createdAt,
    };
  ...
  serializePersistentOrder: (
    apiOrder: APIOrderWithMetaData
  ...
  assetDataOrAssetProxyId: (
    assetData: string | string[] | undefined,
    assetProxyId: string | undefined
  ) => {
  ...
  filterOrders: (
    apiOrders: APIOrderWithMetaData[],
    filters: SRAGetOrdersRequestOpts
  ): APIOrderWithMetaData[] => {
    const {
      traderAddress,
      makerAssetAddress,
    ...
    const matchTraderAddress = (
      order: SignedOrder,
      filterAddress?: string
    ): boolean =>
      filterAddress
        ? order.makerAddress === filterAddress ||
          order.takerAddress === filterAddress
        : true;
    const matchMakerAssetAddress = (
      order: SignedOrder,
      filterAddress?: string
    ): boolean =>
      filterAddress
        ? orderUtils.includesTokenAddress(order.makerAssetData, filterAddress)
        : true;
    const matchTakerAssetAddress = (
      order: SignedOrder,
      filterAddress?: string
    ): boolean =>
      filterAddress
        ? orderUtils.includesTokenAddress(order.takerAssetData, filterAddress)
        : true;
    const matchMakerAssetProxyId = (
      order: SignedOrder,
      filterAddress?: string
    ): boolean =>
      filterAddress
        ? assetDataUtils.decodeAssetDataOrThrow(order.makerAssetData)
            .assetProxyId === makerAssetProxyId
        : true;
    const matchTakerAssetProxyId = (
      order: SignedOrder,
      filterAddress?: string
    ): boolean =>
      filterAddress
        ? assetDataUtils.decodeAssetDataOrThrow(order.takerAssetData)
            .assetProxyId === takerAssetProxyId
        : true;
  ...
  async splitOrdersByPinningAsync(
    _connection: Connection,
    signedOrders: SignedOrder[]
  ): Promise<PinResult> {
    console.log("splitOrdersByPinningAsync", signedOrders);
    return { pin: [], doNotPin: [] };
  },
};

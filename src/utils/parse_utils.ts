...
export const parseUtils = {
  parseRequestForExcludedSources(
    ...
    const excludedIds = request.excludedSources
      ? request.excludedSources.split(",")
      : [];
    const includedIds = request.includedSources
      ? request.includedSources.split(",")
      : [];
    ...
    if (excludedIds.length > 0) {
      return {
        excludedSources: parseUtils.parseStringArrForERC20BridgeSources(
          excludedIds
        ),
        includedSources: [],
        // Exclude open orderbook if 'Mesh' is excluded.
        nativeExclusivelyRFQT:
          excludedIds.includes("Mesh") &&
          !excludedIds.includes("0x") &&
          !excludedIds.includes("Native"),
      };
    }
    ...
    if (includedIds.includes("RFQT")) {
      ...
      return {
        excludedSources: [],
        includedSources: parseUtils.parseStringArrForERC20BridgeSources([
          "0x",
          ...includedIds,
        ]),
        nativeExclusivelyRFQT:
          !includedIds.includes("0x") && !includedIds.includes("Native"),
      };
    }
    ...
    return Object.keys(
      Object.assign(
        ...
        ...sources
          ...
          .filter((source) =>
            Object.values(ERC20BridgeSource).includes(
              source as ERC20BridgeSource
            )
          )
          .map((s) => ({ [s]: s }))
      )
